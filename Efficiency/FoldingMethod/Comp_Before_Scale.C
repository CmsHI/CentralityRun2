...
//}}}
	...
	TString aka[NOF] = {"PR326617", "Hydjet_Cymbal5F", "AMPT_String",
							"AMPT_NoString", "EPOS"};
...
//Get histogram{{{
	...
	{
		fin[ifile] = new TFile(Form("../../HistFiles/PbPb2018_%s_histo.root", aka[ifile].Data()), "READ");
		for(Int_t ivar = 0; ivar < NVar; ivar++)
		{
			...
			FormTH1Marker(h1[ifile][ivar], ifile, ifile, 1.4);
			Double_t ScaleFactor = h1[ifile][ivar]->Integral();
			h1[ifile][ivar]->Scale(1./ScaleFactor);
			h1[ifile][ivar]->Scale(1., "width");
			for(Int_t i = 0; i < 2; i++)
			{
				for(Int_t j = 0; j < 2; j++)
				{
					FormTH1Marker(h2[ifile][ivar][i][j], ifile, ifile, 1.4);
					h2[ifile][ivar][i][j]->Scale(1./ScaleFactor);
					h2[ifile][ivar][i][j]->Scale(1., "width");
				}
			}
		}
	}
...
//total dist{{{
		...
		for(Int_t ifile = 0; ifile < NOF; ifile++)
		{
			h1[ifile][ivar]->Draw("samepe");
			leg1->AddEntry(h1[ifile][ivar], Form("%s", aka[ifile].Data()), "p");
		}
		leg1->Draw();
		c1[ivar]->SaveAs(Form("BeforeScale/%s_dist_noselection_%s.pdf", VarName[ivar].Data(), aka[0].Data()));
//}}}
...
//total dist zoom{{{
		...
		{
			h1[ifile][ivar]->Draw("samepe");
			leg2->AddEntry(h1[ifile][ivar], Form("%s", aka[ifile].Data()), "p");
		}
		leg2->Draw();
		c2[ivar]->SaveAs(Form("BeforeScale/%s_dist_noselection_zoom_%s.pdf", VarName[ivar].Data(), aka[0].Data()));
//}}}
...
//total ratio{{{
		...
		{
			h1[ifile][ivar]->Divide(h1[ifile][ivar], h1[0][ivar], 1, 1, "b");
			h1[ifile][ivar]->Draw("samepe");
			leg3->AddEntry(h1[ifile][ivar], Form("%s", aka[ifile].Data()), "p");
		}
		leg3->Draw();
		SetLine(1, 0, 1, VarMaxC[ivar], 1, 0, 2);
		c3[ivar]->SaveAs(Form("BeforeScale/%s_ratio_noselection_%s.pdf", VarName[ivar].Data(), aka[0].Data()));
//}}}
...
//total ratio zoom{{{
		...
		{
			h1[ifile][ivar]->Draw("samepe");
			leg4->AddEntry(h1[ifile][ivar], Form("%s", aka[ifile].Data()), "p");
		}
		leg4->Draw();
		SetLine(1, 0, 1, NormRangeCut[ivar], 1, 0, 2);
		c4[ivar]->SaveAs(Form("BeforeScale/%s_ratio_noselectioni_zoom_%s.pdf", VarName[ivar].Data(), aka[0].Data()));
//}}}
...
//selected dist{{{
		for(Int_t i = 0; i < 2; i++)
		{
			for(Int_t j = 0; j < 2; j++)
			{
				...
				for(Int_t ifile = 0; ifile < NOF; ifile++)
				{
					h2[ifile][ivar][i][j]->Draw("samepe");
					leg5->AddEntry(h2[ifile][ivar][i][j], Form("%s", aka[ifile].Data()), "p");
				}
				leg5->Draw();
				c5[ivar][i][j]->SaveAs(Form("BeforeScale/%s_dist_%s_%s_%s.pdf", VarName[ivar].Data(), PV[i].Data(), Coin[j].Data(), aka[0].Data()));
			}
		}
//}}}
...
//selected dist zoom{{{
		for(Int_t i = 0; i < 2; i++)
		{
			for(Int_t j = 0; j < 2; j++)
			{
				...
				htmp6->SetAxisRange(0, NormRangeCut[ivar], "X");
				...
				{
					h2[ifile][ivar][i][j]->Draw("samepe");
					leg6->AddEntry(h2[ifile][ivar][i][j], Form("%s", aka[ifile].Data()), "p");
				}
				leg6->Draw();
				c6[ivar][i][j]->SaveAs(Form("BeforeScale/%s_dist_%s_%s_zoom_%s.pdf", VarName[ivar].Data(), PV[i].Data(), Coin[j].Data(), aka[0].Data()));
			}
		}
//}}}
...
//selected ratio{{{
		for(Int_t i = 0; i < 2; i++)
		{
			for(Int_t j = 0; j < 2; j++)
			{
				...
				for(Int_t ifile = 1; ifile < NOF; ifile++)
				{
					h2[ifile][ivar][i][j]->Divide(h2[ifile][ivar][i][j], h2[0][ivar][i][j], 1, 1, "b");
					h2[ifile][ivar][i][j]->Draw("samepe");
					leg7->AddEntry(h2[ifile][ivar][i][j], Form("%s", aka[ifile].Data()), "p");
				}
				leg7->Draw();
				SetLine(1, 0, 1, VarMaxC[ivar], 1, 0, 2);
				c7[ivar][i][j]->SaveAs(Form("BeforeScale/%s_ratio_%s_%s_%s.pdf", VarName[ivar].Data(), PV[i].Data(), Coin[j].Data(), aka[0].Data()));
			}
		}
//}}}
...
//selected ratio zoom{{{
		for(Int_t i = 0; i < 2; i++)
		{
			for(Int_t j = 0; j < 2; j++)
			{
				...
				htmp8->SetAxisRange(0, NormRangeCut[ivar], "X");
				...
				{
					h2[ifile][ivar][i][j]->Draw("samepe");
					leg8->AddEntry(h2[ifile][ivar][i][j], Form("%s", aka[ifile].Data()), "p");
				}
				leg8->Draw();
				SetLine(1, 0, 1, NormRangeCut[ivar], 1, 0, 2);
				c8[ivar][i][j]->SaveAs(Form("BeforeScale/%s_ratio_%s_%s_zoom_%s.pdf", VarName[ivar].Data(), PV[i].Data(), Coin[j].Data(), aka[0].Data()));
			}
		}
//}}}
	}
